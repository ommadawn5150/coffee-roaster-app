...
## 主な機能
  ...
  * 焙煎プロファイルのグラフ化（温度・RoR、固定スケール・スムージング対応）
  * Roast Historyのグラフを並列表示して、過去セッションと現行セッションを比較
  * 過去セッションのBean Name / Green Weight / Roasted Weightをインラインで編集・保存し、Roast Index (Roasted ÷ Green) を表示
...
このアプリケーションは、OpenAIの開発支援エージェント **CODEX** を多用しながら実装・改良されています。UI/UXの磨き込みや各種機能の追加・改善は、CODEXとの対話を通じて継続的に行われています。
